...
out.writerow(['Qty','Digikey', 'Refs','Value', 'Footprint','Purpose'])
...
for group in grouped:
    ...
    for component in group:
        ...
        c = component
        purpose_set.add(c.getField("Purpose"))
        if value is None:
            value=c.getValue()
        elif value!=c.getValue():
            raise ValueError("Value mismatch: ",value)
        if footprint is None:
            footprint=c.getFootprint()
        elif footprint!=c.getFootprint():
            raise ValueError("Footprint mismatch: ",footprint)

    # Fill in the component groups common data
    out.writerow([len(group),
                  fromNetlistText( c.getField("Digikey") ),
                  " ".join(refs),
                  fromNetlistText(value),
